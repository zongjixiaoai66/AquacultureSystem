    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="shuichanCollectionTypes">
                        <el-select v-model="ruleForm.shuichanCollectionTypes" :disabled="ro.shuichanCollectionTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in shuichanCollectionTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="类型" prop="shuichanCollectionValue">
                        <el-input v-model="ruleForm.shuichanCollectionValue"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    shuichanId: false,
                    yonghuId: false,
                    shuichanCollectionTypes: false,
                    insertTime: false,
                },
                ruleForm: {
                    shuichanId: '',
                    yonghuId: '',
                    shuichanCollectionTypes: '',
                    insertTime: '',
                },
                shuichanCollectionTypesOptions : [],
                shuichanOptions : [],
                yonghuOptions : [],
                rules: {
                   shuichanId: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=shuichan_collection_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.shuichanCollectionTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `shuichanCollection/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`shuichanCollection/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.shuichanCollectionCrossAddOrUpdateFlag = false;
                                        this.parent.search();
